    ...
    return [tsfn, js_callback, main_thread_id, tmaps_in, tmap_out, call](ARGS &&...args) -> RET {
      ...
      auto do_call = [&c_ret, &m, &cv, &ready, &rterr, tsfn, main_thread_id, worker_thread_id,
        ...
        try {
          ...
          if (js_ret.IsPromise()) {
            ...
            napi_value on_resolve = Napi::Function::New(env, [env, tmap_out, &c_ret, &m, &cv, &ready, &rterr]
                ...
                try {
                  ...
                  rterr = new std::runtime_error(e.what());
                }
            ...
            napi_value on_reject = Napi::Function::New(env, [env, &m, &cv, &ready, &rterr]
                ...
                Napi::HandleScope store{env};
                // Handle exceptions
                rterr = new std::runtime_error(info[0].ToString());
          ...
          // Handle exceptions
          rterr = new std::runtime_error(err.what());
        }
...
%fragment("SWIG_NAPI_Callback_Void", "header") %{
  #include <thread>
  #include <condition_variable>
  #include <exception>

  template <typename ...ARGS>
  std::function<void(ARGS...)> SWIG_NAPI_Callback_Void(
      Napi::Value js_callback,
      std::function<void(Napi::Env, std::vector<napi_value> &, ARGS...)> tmaps_in
    ) {
      return SWIG_NAPI_Callback_Void(js_callback, tmaps_in,
        [](Napi::Env env, Napi::Function js_callback, std::vector<napi_value> &js_args){
          return js_callback.Call(env.Undefined(), js_args);
      });
    }
  ...
  std::function<void(ARGS...)> SWIG_NAPI_Callback_Void(
    ...
    // $1 is what we pass to the C++ function -> it is a C++ wrapper
    // around the JS callback
    return [tsfn, js_callback, main_thread_id, tmaps_in, call](ARGS &&...args) -> void {
      // Here we are called by the C++ code - we might be in a the main thread (synchronous call)
      // or a background thread (asynchronous call)
      auto worker_thread_id = std::this_thread::get_id();
      std::mutex m;
      ...
      // This is the actual trampoline that allows call into JS
      auto do_call = [&m, &cv, &ready, &rterr, tsfn, main_thread_id, worker_thread_id,
        tmaps_in, call, &args...]
        (Napi::Env env, Napi::Function js_fn) {
        // Here we are back in the main V8 thread, potentially from an async context
        Napi::HandleScope store{env};
        ...
        try {
          ...
          if (js_ret.IsPromise()) {
            ...
            napi_value on_resolve = Napi::Function::New(env, [&m, &cv, &ready]
                (const Napi::CallbackInfo &info) {
                // Nothing to do here - JS returns void
            ...
            napi_value on_reject = Napi::Function::New(env, [env, &rterr, &m, &cv, &ready]
                (const Napi::CallbackInfo &info) {
                Napi::HandleScope store{env};
                // Handle exceptions
                rterr = new std::runtime_error{info[0].ToString()};
