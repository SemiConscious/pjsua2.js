    ...
    'conditions': [
      ...
      ['target_platform == "emscripten" or OS == "linux" or OS =="mac"', {
        'actions': [
          {
            'action_name': 'make',
            'inputs': [ '<(module_root_dir)/deps/pjproject/configure' ],
            'conditions': [
              # ['enable_hdri == "false"', {
              #   'outputs': [
        ...
        'direct_dependent_settings': {
          'include_dirs': [
            '<(module_root_dir)/deps/pjproject/pjlib/include',
            '<(module_root_dir)/deps/pjproject/pjmedia/include',
            '<(module_root_dir)/deps/pjproject/pjnath/include',
            '<(module_root_dir)/deps/pjproject/pjsip/include',
          ],
          'libraries': [
      ...
      ['target_platform != "emscripten" and OS == "win"', {
        'variables': {
          'pjsua2_win_lib': '<(module_root_dir)/deps/pjsua2/Output/lib'
        },
        'actions': [
          {
            'action_name': 'make',
            'inputs': [ '<(module_root_dir)/deps/pjproject/pjproject-vs14.sln' ],
            'outputs': [ '<(module_root_dir)/deps/lib/CORE_<(winlibid)_pjsua2_.lib' ],
            'action': [ '<(module_root_dir)/deps/build.bat', '<(module_path)', '<(winbuildtype)' ]
          }
        ],
        'direct_dependent_settings': {
          'include_dirs': [
          ...
          'inputs': [ '<!@(configure.bat > ../build/configure.log)' ]
        }
      }]
    ]
  }]
